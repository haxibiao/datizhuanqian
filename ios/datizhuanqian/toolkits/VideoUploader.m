...
//  datizhuanqian
//
//  Created by wangyukun on 2019/5/8.
...
@interface VideoUploader : RCTEventEmitter <RCTBridgeModule, NSURLSessionTaskDelegate>
{
...
- (NSArray<NSString *> *)supportedEvents {
  return @[
           @"VideoUploader-progress",
           @"VideoUploader-error",
           @"VideoUploader-cancelled",
           @"VideoUploader-completed"
           ];
}
  ...
  @try {
    ...
    if (exists)
    {
      ...
      if (error == nil)
      {
        ...
        [params setObject:[NSNumber numberWithLong:seconds] forKey:@"duration"];
      }
    }
    resolve(params);
  }
...
didCompleteWithError:(NSError *)error {
  ...
  if (error == nil)
  {
    [self _sendEventWithName:@"VideoUploader-completed" body:data];
  }
  else
  {
    [data setObject:error.localizedDescription forKey:@"error"];
    if (error.code == NSURLErrorCancelled) {
      [self _sendEventWithName:@"VideoUploader-cancelled" body:data];
    } else {
      [self _sendEventWithName:@"VideoUploader-error" body:data];
    }
  }
}
...
totalBytesExpectedToSend:(int64_t)totalBytesExpectedToSend {
  ...
  [self _sendEventWithName:@"VideoUploader-progress" body:@{ @"id": task.taskDescription, @"progress": [NSNumber numberWithFloat:progress] }];
}
