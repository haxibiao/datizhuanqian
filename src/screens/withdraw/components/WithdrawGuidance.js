...
import { Theme, PxFit, SCREEN_WIDTH, WPercent, Api } from 'utils';
...
class WithdrawGuidance extends Component {
	checkUser = async () => {
		...
		if (result && result.data && result.data.user) {
			...
			Api.checkLoginInfo(user.auto_uuid_user, user.auto_phone_user, navigation, user);
		} else {
			Toast.show({
	...
	render() {
		let { tips, navigation, user } = this.props;
		return (
			<View
				style={{
					flex: 1,
