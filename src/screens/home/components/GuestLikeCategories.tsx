...
const RecommendCategories = () => {
    ...
    const GuestLikeCategoriesQuery = useCallback(() => {
        return client.query({
            query: GQL.GuestLikeCategoriesQuery,
    ...
    const getData = async () => {
        const gusetLikeCategoriesQuery = await Helper.exceptionCapture(() => GuestLikeCategoriesQuery());

        const guestLikeCategories = Helper.syncGetter('data.guestLikeCategories', gusetLikeCategoriesQuery[1]) || [];

        setCategories(guestLikeCategories);
        count.current += guestLikeCategories.length;
    };
