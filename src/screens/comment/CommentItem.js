...
class CommentItem extends Component<Props> {
	...
	render() {
		let { comment, navigation, questionId, showCommentModal, replyComment } = this.props;
		let { liked, count_likes, bounce, visible, limit, loadingMore } = this.state;
								...
								<Animated.View style={{ transform: [{ scale: scale }] }}>
									...
									>
										<Iconfont
											...
											size={PxFit(20)}
											color={liked ? Theme.themeRed : Theme.subTextColor}
										/>
									</TouchFeedback>
								</Animated.View>
								{count_likes > 0 && (
