...
class WithdrawNotificationItem extends Component {
    ...
    render() {
        ...
        let withdraw = Helper.syncGetter('withdraw', notification);
        return (
            <TouchableOpacity
                style={styles.item}
                onPress={() => {
                ...
                <View style={styles.bottomInfo}>
                    <Text style={styles.text}>￥{Helper.syncGetter('amount', withdraw)}.00</Text>
                    {notification.type == 'WITHDRAW_SUCCESS' ? (
                        <View>
                            <Text style={{ color: Theme.weixin, paddingVertical: 3 }}>提现成功</Text>
                            <Text style={styles.infoItem}>
                                提现方式：支付宝({Helper.syncGetter('to_account', withdraw)})
                            </Text>
                            <Text style={styles.infoItem}>到账时间：{Helper.syncGetter('updated_at', withdraw)}</Text>
                        </View>
                    ) : (
                        <View>
                            <Text style={{ color: Theme.secondaryColor, paddingVertical: 3 }}>提现失败</Text>
                            <Text style={styles.infoItem}>提现单号：{Helper.syncGetter('biz_no', withdraw)}</Text>
                            <Text style={[styles.infoItem, { lineHeight: PxFit(20) }]}>
                                回执信息：{Helper.syncGetter('remark', withdraw)}
                            </Text>
                        </View>
                    )}
                </View>
                <TouchableOpacity
                    ...
                    onPress={() => {
                        navigation.navigate('withdrawLogDetails', {
                            withdraw_id: Helper.syncGetter('id', withdraw),
                        });
                    }}>
                    <Text>查看详情</Text>
