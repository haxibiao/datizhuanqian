...
const [aWidth] = [Divice.width - 20];
const [left, top] = [0, 0];
const [middleLeft] = [(Divice.width - aWidth) / 2];
								...
								transform: [
									{
										translateY: this.state.offset.interpolate({
											inputRange: [0, 1],
											outputRange: [Divice.height, Divice.height - this.state.aHeight - 34]
										})
									}
								]
							}
	...
	show(title: string, entityList: Array, tipTextColor: string, callback: Object) {
		...
		if (this.state.hide) {
			if (entityList && entityList.length > 0) {
				...
				if (len === 1) {
					this.setState(
						{ title: title, choose0: entityList[0], hide: false, tipTextColor: tipTextColor, aHeight: 180 },
						this.in
					);
				} else if (len === 2) {
					this.setState(
