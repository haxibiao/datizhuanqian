...
import { TouchFeedback, Iconfont, Row } from '@src/components';
...
interface Props {
    ...
    title?: String;
    buttonName?: String;
    playVideoVariables: object;
    action: Function;
}
let OverlayKey: any = null;

const ErrorOverlay = props => {
    const { content, title, buttonName, playVideoVariables, action } = props;
    const [adShow, setAdShow] = useState(false);
    console.log('playVideoVariables :>> ', playVideoVariables);
    return (
                ...
                <View style={{ alignItems: 'center' }}>
                    ...
                    <View style={{ alignItems: 'center', marginTop: PxFit(5), paddingBottom: PxFit(15) }}>
                        <Button
                            style={styles.button}
                            textColor={'#623605'}
                            title={buttonName || '看视频立即领取'}
                            onPress={() => {
                                action ? action() : playVideo({ ...playVideoVariables });
                                hide();
                            }}
                        />
                    </View>
...
const styles = StyleSheet.create({
    ...
    headerImage: {
        width: (Device.WIDTH * 0.28 * 487) / 375,
